export type BagMap = Map<string, BagContent[]>;
...
const fixContains = (definition: string): BagContent[] => {
  if (!definition) {
    return [];
  }

  const groups = definition.match(/([\d]) ([a-z ]+) bag([s|\.]?)/g);

  if (!groups) {
    return [];
  }

  return groups.map(bag => {
    const [count, n1, n2] = bag.split(' ');
    return {
...
const transform = (data: string): BagMap => {
  let rows = data.split('\n');
  const map = new Map<string, BagContent[]>();

  rows.forEach(rule => {
    const [color, contains] = rule.split(' bags contain ');
    map.set(color, fixContains(contains));
  });

  return map;
};

export const testInput = transform(`shiny gold bags contain 2 dark red bags.
...
// export const testInput = transform(`light red bags contain 1 bright white bag, 2 muted yellow bags.
...
export const input = transform(`clear maroon bags contain 1 dull lavender bag.
