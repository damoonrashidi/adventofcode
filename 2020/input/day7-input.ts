...
const fixContains = (definition: string): string[] => {
  if (!definition) {
    return [];
  }
  ...
  const contains = groups.map(bag => {
    const [, n1, n2] = bag.split(' ');
    return `${n1} ${n2}`;
  });
...
const transform = (data: string): Bag[] => {
  let rows = data.split('\n');

  return rows.map(rule => {
    const [color, contains] = rule.split(' bags contain ');

    return { color, contains: fixContains(contains) };
  });
};
